...
function createRideDetailsSection(iconName, title, value) {
    ...
    icon.className = 'icon';
    icon.innerHTML = `<svg class="icon"> <use href="/icons/icons.svg#${iconName}" /></svg>`;
...
function createRideDetails(ride, containerDiv) {
    ...
    image.src = "http://127.0.0.1:8081/images" + ride.imageURL;
    image.alt = ride.name;
    ...
    if (ride.minHeight > 0) {
        infoDiv.appendChild(createRideDetailsSection('icon-min-height',
            'Minimum Height', `${ride.minHeight}in`));
    }

    infoDiv.appendChild(createRideDetailsSection('icon-duration',
